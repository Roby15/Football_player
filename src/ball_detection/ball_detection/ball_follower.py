...
class BallFollower(Node):
    ...
    def ball_position_callback(self, msg):
        ...
        if self.ball_detected:
            ...
            self.perform_kick()  # Trigger kick if the ball is not detected
    ...
    def move_towards_ball(self):
        if self.ball_centered:
            ...
            if self.ball_position.y < 100:  # Threshold for the y position (adjust as necessary)
                  # Stop if we're close to the ball
                self.stop_movement()
                self.perform_kick()
    ...
    def perform_kick(self):
        # Perform the kick action when the ball is not detected
        twist = Twist()
        
        # Apply a higher linear velocity to simulate the kick
        twist.linear.x = 5.0  # Kick speed (adjust as needed)

        # Optionally, add a short duration for the kick action
        time.sleep(1)  # Kick duration
        
        # Stop the robot after kicking the ball
        self.stop_movement()

        # Publish the kick twist
        self.publisher.publish(twist)
